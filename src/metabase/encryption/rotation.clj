                                    ...
                                    (let [raw       (get row column)
                                      ...
                                      (when (and (some? value)
                                        ...
                                        (let [decrypted (encryption.impl/maybe-decrypt value)]
                                          ;; If still looks encrypted after decryption attempt, the key is wrong.
                                          (when (and (string? decrypted)
                                                     (= decrypted value)
                                                     (encryption.impl/possibly-encrypted-string? decrypted))
                                            (throw (ex-info (trs "Can''t decrypt app db with MB_ENCRYPTION_SECRET_KEY")
                                                            {:model model :column column :row row})))
